...
class TestPairwise(unittest.TestCase):
    def test_style_code_is_authoritative(self):
        ...
        b = listing("goat", "AM90 White Black", 99, style_code="HM0089-100")
    ...
    def test_conflicting_style_codes_never_merge(self):
        a = listing("nike", "Nike Air Max 90 Men's Shoes", 130, style_code="HM0089-100")
        b = listing("stadiumgoods", "Nike Air Max 90 - Men's", 120, style_code="CN8490-002")
    ...
    def test_version_numbers_block_a_fuzzy_merge(self):
        ...
        b = listing("kickscrew", "New Balance Men's 990v5 Grey", 175)
        self.assertFalse(is_same_product(a, b))

    def test_qualifier_words_still_merge(self):
        a = listing("newbalance", "New Balance 990v6 Made in USA Men's Grey", 200)
        b = listing("kickscrew", "New Balance Men's 990v6 Grey", 190)
        self.assertTrue(is_same_product(a, b))

    def test_different_brands_never_merge(self):
        a = listing("stadiumgoods", "Nike Air Max 90 - Men's - White/Black", 120)
        b = listing("stadiumgoods", "adidas Air Max 90 - Men's - White/Black", 120)
        self.assertFalse(is_same_product(a, b))

    def test_mens_and_womens_are_separate_products(self):
        a = listing("stadiumgoods", "Nike Pegasus 41 - Men's - Black/White", 140)
        b = listing("stadiumgoods", "Nike Pegasus 41 - Women's - Black/White", 140)
        self.assertFalse(is_same_product(a, b))

    def test_different_colorways_do_not_merge(self):
        a = listing("stadiumgoods", "Nike Dunk Low Retro - Men's - White/Black", 120)
        b = listing("stadiumgoods", "Nike Dunk Low Retro - Men's - Green/Cream", 120)
        self.assertFalse(is_same_product(a, b))
...
class TestClustering(unittest.TestCase):
    def test_one_shoe_four_sites_one_cluster(self):
        ...
        self.assertEqual(product.best_offer().source, "kickscrew")
        self.assertEqual(product.best_offer(include_shipping=False).source, "goat")

    def test_distinct_shoes_stay_distinct(self):
        items = air_max_90_across_sites() + [
            listing("stadiumgoods", "Nike Air Max 95 - Men's - Grey", 175),
            listing("stadiumgoods", "Nike Air Max 90 - Men's - Infrared", 130),
        ]
    ...
    def test_cheapest_accounts_for_shipping(self):
        items = [
            listing("goat", "Nike Air Max 90 Men's", 109.99, shipping=25.0),
            listing("stadiumgoods", "Nike Air Max 90 - Men's - White/Black", 119.99, shipping=0.0),
        ]
